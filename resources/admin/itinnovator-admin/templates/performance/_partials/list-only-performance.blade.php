...
<tr class="m-datatable__row">
  <td>{{ $o->id }}</td>
        ...
        <span>
                    ...
                    <td>
            @foreach ($o->performance_category as $t)
              <span>{{ $t->name }}</span>
            @endforeach
          </td>
                <td>
        <span>
                      {{ model($o->performance_dates, 'name') }}
                  </span>
      </td>
        <td>
    <div class="dropdown">
      ...
      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton_{{ $o->id }}" x-placement="bottom-start" style="position: absolute; transform: translate3d(0px, 37px, 0px); top: 0px; left: 0px; will-change: transform;">
        <a class="dropdown-item" href="{{ AdminURL('performance/edit/' . $o->id) }}">{{ t('Edit') }}</a>
        <a class="dropdown-item" href="{{ AdminURL('performance/delete/' . $o->id) }}">{{ t('Delete') }}</a>
