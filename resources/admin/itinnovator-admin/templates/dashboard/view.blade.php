@extends('layouts.dashboard')
@section('content')
<div class="content" id="dash">
  <div class="flex space-between _top_s _stb">
    <div class="box flex space-between top-box">
      <div class="icon-block">
        <i class="material-icons">wc</i>
      </div>
      <div class="block-content">
        <div id="live_users" class="digit fetch-user">
          @{{ online_users }}
        </div>
        <h4>Active users</h4>
        <a href="https://analytics.google.com/analytics/web/#realtime/rt-overview" target="_blank">View in Google</a>
      </div>
    </div>
    <div class="box flex space-between top-box">
      <div class="icon-block">
        <i class="material-icons">
        table_chart
        </i>
      </div>
      <div class="block-content">
        <div class="digit">{{ $context->post->all()->count() }}</div>
        <h4>Posts</h4>
        <a href="{{ AdminURL('post') }}">View</a>
      </div>
    </div>
    <div class="box flex space-between top-box">
      <div class="icon-block">
        <i class="material-icons">
        grid_on
        </i>
      </div>
      <div class="block-content">
        <div class="digit">{{ $context->block->all()->count() }}</div>
        <h4>Blocks</h4>
        <a href="{{ AdminURL('block') }}">View</a>
      </div>
    </div>
    <div class="box flex space-between top-box">
      <div class="icon-block">
        <i class="material-icons">
        supervised_user_circle
        </i>
      </div>
      <div class="block-content">
        <div class="digit">{{ $context->admin_user->all()->count() }}</div>
        <h4>Admin Users</h4>
        <a href="{{ AdminURL('admin/user') }}">View</a>
      </div>
    </div>
  </div>
  @if (config('adlara.dashboard_analytics'))
  <div class="flex space-between _stb">
    <div class="box">
      <div class="flex space-between">
        <h4>Daily Visits</h4>
        <p>Updated just now</p>
      </div>
      <canvas id="dailyVisits"></canvas>
    </div>
    <div class="box">
      <div class="flex space-between">
        <h4>Top Devices</h4>
        <p>Updated just now</p>
      </div>
      <canvas id="topDevices"></canvas>
    </div>
  </div>
  <div class="_stb flex space-between">
    <div class="box _lu">
      <div class="flex space-between">
        <h4>Most Viewed</h4>
      </div>
      @if (count($most_visited))
      <table class="table table-mv">
        <tr>
          <th>Page</th>
          <th>Page Views</th>
        </tr>
        @foreach ($most_visited as $mv)
        <tr>
          <td>{{ $mv['url'] }}</td>
          <td>{{ $mv['pageViews'] }}</td>
        </tr>
        @endforeach
      </table>
      @endif
    </div>
    <div class="box">
      <div class="flex space-between">
        <h4>Top Referrers</h4>
        <p>Updated just now</p>
      </div>
      @if (count($top_referrers))
      <table class="table table-mv">
        <tr>
          <th>URL</th>
          <th>Views</th>
        </tr>
        @foreach ($top_referrers as $mv)
        <tr>
          <td>{{ $mv['url'] }}</td>
          <td>{{ $mv['pageViews'] }}</td>
        </tr>
        @endforeach
      </table>
      @endif
    </div>
    <div class="box">
      <div class="flex space-between">
        <h4>Top Browsers</h4>
      </div>
      @if (count($top_browsers))
      <table class="table table-mv">
        <tr>
          <th>Browser</th>
          <th>Sessions</th>
        </tr>
        @foreach ($top_browsers as $mv)
        <tr>
          <td><i class="fa fa-{{ strtolower($mv['browser']) }}"></i> {{ $mv['browser'] }}</td>
          <td>{{ $mv['sessions'] }}</td>
        </tr>
        @endforeach
      </table>
      @endif
    </div>
  </div>
  @else
  <div class="container">    
    <div class="alert alert-danger">
      Open config/adlara.php file and set "dashboard_analytics" to true to see your dashboard analytics. Before that make sure that you have obtained Google Analytics key and made changes to other setting as described here.
      <a href="https://github.com/spatie/laravel-analytics" target="_blank">See settings</a>
    </div>
  </div>
  @endif
</div>
@endsection
@section('footer_script')
<script>
var elem = document.getElementById('dash');
if (elem) {
  const app = new Vue({
    el: '#dash',
    data () {
      return {
        login: false,
        user: null,
        online_users: 0,
        interval: null,
        devices: [],
        os: []
      }
    },
    methods: {
      getLiveUsers: function() {
        axios.get(ADMIN_URL + '/dashboard/live/users')
        .then((res) => {
          this.online_users = res.data;
        });
      },
      getTopDevices() {
        axios.get(ADMIN_URL + '/dashboard/top/os')
        .then((res) => {
          labels = [];
          var top_os = res.data;
          Object.keys(top_os.labels).forEach(function(v, i) {
            labels.push(v);
          });

          c_data = {
            'id': 'topDevices',
            'type': 'bar',
            'labels': labels,
            'sets': top_os.sets
          };
          myChart(c_data);
        });
      },
      getDailyVisits() {
        axios.get(ADMIN_URL + '/dashboard/daily/visits')
        .then((res) => {
          c_data = {
            'id': 'dailyVisits',
            'type': 'line',
            'labels': res.data.labels,
            'sets': res.data.sets
          }
          myChart(c_data);
        });
      }
    },
    created() {
      this.getLiveUsers();
      this.getTopDevices();
      this.getDailyVisits();
      this.interval = setInterval(function () {
        this.getLiveUsers();
      }.bind(this), 50000);
    },
  });
}

function myChart(data) {
  var ctx = document.getElementById(data.id);
  datasets = [];
  if (data.sets.length) {
    for (i = 0; i < data.sets.length; i++) {
      datasets[i] = {
        'label': data.sets[i].label
      };
      datasets[i]['data'] = data.sets[i].data;
      datasets[i]['backgroundColor'] = [data.sets[i].bg];
      datasets[i]['borderWidth'] = 1;
    }
  }

  var myChart = new Chart(ctx, {
    type: data.type,
    data: {
      labels: data.labels,
      datasets: datasets,
    },
    options: {
      scales: {
        yAxes: [{
          ticks: {
            beginAtZero: true
          }
        }]
      }
    }
  });
}
</script>
@endsection
